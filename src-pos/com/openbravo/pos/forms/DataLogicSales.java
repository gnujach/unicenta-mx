...
// Campo Marca 25/11/13
                new Field(AppLocal.getIntString("label.prodMarca"), Datas.STRING, Formats.STRING, false, false, true),
// Campo Mayoreo 3/12/13
                new Field(AppLocal.getIntString("label.prodpricesell1"), Datas.DOUBLE, Formats.CURRENCY, false, true, true));
...
// **
	...
	return (ProductInfoExt) new PreparedSentence(s
		, "SELECT ID, REFERENCE, CODE, NAME, ISCOM, ISSCALE, PRICEBUY, PRICESELL, TAXCAT, CATEGORY, ATTRIBUTESET_ID, IMAGE, ATTRIBUTES, ISKITCHEN, ISSERVICE, DISPLAY, MARCA, PRICESELL1 " +
    ...
    //Marcas
    public final List<MarcasInfo> getMarcasCategories() throws BasicException {
        return new PreparedSentence(s
            , "SELECT ID, NAME,  FROM MARCAS WHERE NULL ORDER BY NAME"
            , null
            , MarcasInfo.getSerializerRead()).list();
    }
...
// Setting JVM -Xms & -Xmx only partial solution     
	...
	return new PreparedSentence(s
		, "SELECT P.ID, P.REFERENCE, P.CODE, P.NAME, P.ISCOM, P.ISSCALE, P.PRICEBUY, P.PRICESELL, P.TAXCAT, P.CATEGORY, P.ATTRIBUTESET_ID, P.IMAGE, P.ATTRIBUTES, P.ISKITCHEN, P.ISSERVICE, P.DISPLAY, P.MARCA, P.PRICESELL1 " +
...
// End of Promotion
  ...
  //Agregado MarcasInfo 28/11/13
   public final MarcasInfo getMarcasInfo(String id) throws BasicException {
        return (MarcasInfo) new PreparedSentence(s
        , "SELECT ID, NAME FROM MARCAS ORDER BY NAME WHERE ID = ?"
        , SerializerWriteString.INSTANCE
        , MarcasInfo.getSerializerRead()).find(id);
    }
// Products list
// ADDED JG 20.12.10 ISKITCHEN - Kitchen Print + 25.06.2011 ISSERVICE - ISSERVICE
// ADDED JG 13 NOV 12 DISPLAY - Button display text for HTML rendering***
...
// ADDED JG 13 NOV 12 DISPLAY - Button display text for HTML rendering***
 ...
 public Object readValues(DataRead dr) throws BasicException {
    ...
    //Agregamos metodo para retomar las marcas 28/11/14
    public final SentenceList getMarcasList() {
        return new StaticSentence(s
            , "SELECT ID, NAME FROM MARCAS ORDER BY NAME"
            , null
            , MarcasInfo.getSerializerRead());
    }
    ...
    public CustomerInfoExt findCustomerExt(String card) throws BasicException {
        return (CustomerInfoExt) new PreparedSentence(s
                , "SELECT ID, TAXID, SEARCHKEY, NAME, CARD, TAXCATEGORY, NOTES, MAXDEBT, VISIBLE, CURDATE, CURDEBT" +
                  ...
                  ", ADDRESS, ADDRESS2, POSTAL, CITY, REGION, COUNTRY, MAYOREO" +
...
// ADDED JG 13 NOV 12 DISPLAY - Button display text for HTML rendering***
    public final SentenceList getProductCatQBF() {
 	return new StaticSentence(s
		, new QBFBuilder(
			"SELECT P.ID, P.REFERENCE, P.CODE, P.NAME, P.ISCOM, P.ISSCALE, P.PRICEBUY, P.PRICESELL, P.CATEGORY, P.TAXCAT, P.ATTRIBUTESET_ID, P.IMAGE, P.STOCKCOST, P.STOCKVOLUME, CASE WHEN C.PRODUCT IS NULL THEN " + s.DB.FALSE() + " ELSE " + s.DB.TRUE() + " END, C.CATORDER, P.ATTRIBUTES, P.ISKITCHEN, P.ISSERVICE, P.DISPLAY, P.MARCA, P.PRICESELL1 " +
...
// ADDED JG 13 NOV 12 DISPLAY - Button display text for HTML rendering***
	...
	return new SentenceExecTransaction(s) {
		...
		public int execInTransaction(Object params) throws BasicException {
			...
			int i = new PreparedSentence(s
				, "INSERT INTO PRODUCTS (ID, REFERENCE, CODE, NAME, ISCOM, ISSCALE, PRICEBUY, PRICESELL, CATEGORY, TAXCAT, ATTRIBUTESET_ID, IMAGE, STOCKCOST, STOCKVOLUME, ATTRIBUTES, ISKITCHEN, ISSERVICE, DISPLAY, MARCA, PRICESELL1) VALUES (?,?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
	...
	return new SentenceExecTransaction(s) {
		...
		public int execInTransaction(Object params) throws BasicException {
			Object[] values = (Object[]) params;
			int i = new PreparedSentence(s
				, "UPDATE PRODUCTS SET ID = ?, REFERENCE = ?, CODE = ?, NAME = ?, ISCOM = ?, ISSCALE = ?, PRICEBUY = ?, PRICESELL = ?, CATEGORY = ?, TAXCAT = ?, ATTRIBUTESET_ID = ?, IMAGE = ?, STOCKCOST = ?, STOCKVOLUME = ?, ATTRIBUTES = ?, ISKITCHEN = ?, ISSERVICE = ?, DISPLAY = ?, MARCA = ?, PRICESELL1 = ? WHERE ID = ?"
...
// ADDED JG 03.07.11 Payment Notes
    ...
    //Add Marcas 22/11/13
    public final SentenceExec getCatalogMarcasAdd() {
        return new StaticSentence(s
                , "INSERT INTO MARCAS (NAME) SELECT ID, " + s.DB.INTEGER_NULL() 
                , SerializerWriteString.INSTANCE);
    }
    ...
    //add marcas model
     public final SentenceExec getCatalogMarcasDel() {
        return new StaticSentence(s
                , "DELETE FROM MARCAS WHERE ID = ANY (SELECT ID FROM MARCAS WHERE ID = ?)"
                , SerializerWriteString.INSTANCE);
    }
    ...
    //Agregar Definicion de Table Marcas
    public final TableDefinition getTableMarcas() {
        return new TableDefinition(s,
            "MARCAS"
            , new String[] {"ID", "NAME"}
    ...
    protected static class CustomerExtRead implements SerializerRead {
        ...
        public Object readValues(DataRead dr) throws BasicException {
            ...
            c.setMayoreo(dr.getBoolean(24).booleanValue());            
            return c;
        }
    }  
}
